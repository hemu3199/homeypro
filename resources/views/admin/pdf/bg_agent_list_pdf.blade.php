...
<html>
    ...
    <body>
        <center><h1 class="text-center">Bg Agent List</h1></center>


        <h2>Total Agents: {{$bgagentcount}}</h2>
        @if ($bgagentcount > 0) 
            <table id="customers" class="table table-bordered">
                ...
                <tbody>
                    @foreach($bgagentlist as $key => $row)
                    @php
                        $bgagent_status = DB::table('agents')->where(['agent_id' => $row->agent_id])->pluck('status')->first();
                    @endphp
                        <tr>
