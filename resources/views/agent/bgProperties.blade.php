                ...
                <div class="card">
                    <div class="card-header  border-0">
                        <h4 class="card-title">Properties List</h4>
                            ...
                            <a class="btn btn-primary btn-icon" href="{{route('agent.bg-properties-pdf')}}"  style="margin-left: 10px; size: 40px;">
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table  table-vcenter text-nowrap table-bordered border-bottom" id="hr-table">
                                ...
                                @foreach($totalproperties as $key => $row)
                                    <tr>
                                        ...
                                        <td><a href="{{ route('agent.bgproperties_details',$row->property_id)}}" onmouseover="this.style.color='#000';" onmouseout="this.style.color='#000';">{{$row->name}}</a></td>
                                        <td>{{$row->address}}</td>
                                        @php
                                            ...
                                            @endif
                                        <td>
                                            <span class="badge rounded-pill bg-primary " style="padding:8px;font-size:12px;">{{$row->categories}}</span></td>
                                           <td> @if(!empty($row->price))
                                            
                                        <span class="badge rounded-pill bg-primary mt-2 ml-4" style="padding:8px;font-size:12px;">{{$row->price}}</span>
                                            @else
                                          
                                        <span class="badge rounded-pill bg-dark mt-2 ml-4" style="padding:8px;font-size:12px;">NA</span>
                                        @endif
                            ...
                            <nav aria-label="Page navigation">{!! $totalproperties->links() !!}</nav>
                        </div>
                    </div>
                </div>
            </div>
