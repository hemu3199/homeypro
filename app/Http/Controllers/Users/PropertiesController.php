...
class PropertiesController extends Controller
{
    ...
    return view('user.properties_list',compact('intersted'));
    ...
    {
     return view('users.add_property');
    }
     public function add_property_view()
 {
    return view('user.property_add');
 }
...
public function add_propertyfun(Request $request)
{
        ...
        // Handle the exception and display the error message
                ...
                function edit_properties(Request $request,$id)
                {
                    ...
                    return view('user.properties_edit',compact('property_details','room','roomcount','houseplanscount','houseplans'));
    ...
    if($request->isMethod('post'))
    {
                                    ...
                                    "mic"=>$mic,
                                     ...
                                     if ($request->hasFile('room1.' . $key)) {
                                        foreach ($request->file('room1.' . $key) as $m_image) {
                                            $m_imagename = $value['room_id'].time() . '_' . $m_image->getClientOriginalName();
                                            $m_image->move(public_path('uploads/properties'), $m_imagename);
                                            
                                            $room_images = new Room_images();
                                            $room_images->property_id = $id;
                                            $room_images->room_id = $value['room_id'];
                                            $room_images->image_name = $m_imagename;
                                            $room_images->save();
                                        }
                                    }
        ...
        Function deletehousesection(Request $request,$id)
        {
            ...
            return back()->with('message', 'House Deleted Successfully!!');
        }
